    ...
    for field_name, field_info in fields.items():
        ...
        if field_type_parent is list:
            ...
            if inspect.isclass(element_type) and issubclass(element_type, BaseModel):
                model_description += f"{indent}- {field_name} (list): {description+'.' or ''} Each item in this list should follow the structure:\n"
                model_description += describe_pydantic_model(
                    element_type, indent + "\t")
            else:
                model_description += f"{indent}- {field_name} (list): {description+'.' or ''} Each item in this list should be of type {element_type.__name__}\n"
        elif inspect.isclass(field_type) and issubclass(field_type, BaseModel):
            model_description += f"{indent}- {field_name} (obj): {description+'.' or '' } This should follow the structure:\n"
            model_description += describe_pydantic_model(
                field_type, indent + "\t")
        else:
